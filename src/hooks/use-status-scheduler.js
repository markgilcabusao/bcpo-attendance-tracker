 ...
 */
export function useStatusScheduler(onTaskExecute, checkOutOfficer) {
    ...
    const addTask = useCallback((officerId, officerName, scheduledStatus, scheduledTime) => {
        // Cancel any existing pending task for this officer
        setTasks(prevTasks => prevTasks.map(task => task.officerId === officerId && task.status === 'pending'
    ...
    const cancelTask = useCallback((taskId) => {
        setTasks(prevTasks => prevTasks.map(task => task.id === taskId && task.status === 'pending'
            ? { ...task, status: 'cancelled', cancelledAt: new Date().toISOString() }
            : task));
    }, []);
