...
const saveToLocalBackup = (records) => {
    try {
...
// Hook Implementation
// ============================================================================
export function useSupabaseDutyRecords(retryConfig = DEFAULT_RETRY_CONFIG) {
    ...
    const dutyRecordsChangeCallbacks = useRef(new Set());
    const supabaseAvailable = isSupabaseConfigured();
    // Callback registration for duty records changes
    const onDutyRecordsChange = useCallback((callback) => {
        dutyRecordsChangeCallbacks.current.add(callback);
        return () => {
            dutyRecordsChangeCallbacks.current.delete(callback);
        };
    }, []);
    const notifyDutyRecordsChange = useCallback((officerId, status, dutyRecord) => {
    ...
    // ============================================================================
    const fetchDutyRecords = useCallback(async (options) => {
    ...
    // ============================================================================
    const fetchDutyRecordsForOfficer = useCallback(async (officerId, options) => {
        if (!supabaseAvailable) {
    ...
    // ============================================================================
    const fetchDutyRecordsForDate = useCallback(async (date) => {
    ...
    // ============================================================================
    const checkInOfficer = useCallback(async (officerId, notes) => {
        if (!supabaseAvailable) {
            setError('Supabase is not configured');
            return null;
        }
    ...
    // ============================================================================
    const addDutyRecord = useCallback(async (record) => {
        if (!supabaseAvailable) {
            setError('Supabase is not configured');
            return null;
        }
    ...
    // Update Duty Record
    // ============================================================================
    const updateDutyRecord = useCallback(async (id, record) => {
        if (!supabaseAvailable) {
            setError('Supabase is not configured');
            return null;
        }
    ...
    // Delete Duty Record
    // ============================================================================
    const deleteDutyRecord = useCallback(async (id) => {
        if (!supabaseAvailable) {
            setError('Supabase is not configured');
            return false;
        }
    ...
    // Fetch Monthly Stats
    // ============================================================================
    const fetchMonthlyStats = useCallback(async (month) => {
        if (!supabaseAvailable) {
            setError('Supabase is not configured');
            return;
        }
    ...
    // ============================================================================
    const getOfficersOnDuty = useCallback(async (date) => {
        if (!supabaseAvailable) {
            return [];
        }
    ...
    // Get Duty Stats
    // ============================================================================
    const getDutyStats = useCallback(async (startDate, endDate) => {
        if (!supabaseAvailable) {
            return [];
        }
    ...
    useEffect(() => {
        ...
        const setupSubscription = () => {
            ...
            subscriptionRef.current = supabase
                ...
                .on('postgres_changes', { event: '*', schema: 'public', table: 'duty_records' }, (payload) => {
                if (!isMountedRef.current)
                    return;
                if (payload.eventType === 'INSERT') {
                    ...
                    setDutyRecords(prev => prev.map(r => (r.id === updatedRecord.id ? updatedRecord : r)));
