...
export function useCalendarState(): UseCalendarStateReturn {
  ...
  const loadFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      if ((data as any)?.preference_value && isMountedRef.current) {
        const state = (data as any).preference_value as {
          currentMonth?: string
  ...
  const saveToSupabase = useCallback(
    async (state: CalendarState) => {
      ...
      try {
        ...
        const { error: supabaseError } = await supabase.from('user_preferences' as any).upsert(
          {
